...
- RBAC (T-018): под Bearer — `GET /rbac/permissions` (30 шт. из seed), `GET /rbac/roles?tenantSlug=demo` — роль «Демо-аудиторы» с 30 ячейками (edit: engagement/finding, view: остальное); без `tenantSlug` тенантские роли не видны (RLS).
