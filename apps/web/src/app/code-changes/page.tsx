...
export default async function CodeChangesPage() {
  ...
  const [t, tenantSlug] = await Promise.all([getTranslations('codeChanges'), getActiveTenantSlug()]);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 p-6 pt-12">
      ...
      <form action={createChangeAction} data-testid="change-create" className="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-white p-4 shadow-sm">
        ...
        <select name="risk" defaultValue="medium" className={inputCls}>
          {RISKS.map((r) => (
            <option key={r} value={r}>{t(`risks.${r}`)}</option>
          ))}
        </select>
        <button type="submit" className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-on-primary transition-colors duration-150 hover:bg-accent/90 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
          {t('add')}
        </button>
      </form>
              ...
              {changes.map((c) => (
                <tr key={c.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_TONE[c.status]}`}>{t(`st.${c.status}`)}</span>
                  </td>
                  <td className="px-4 py-3">
                    {NEXT[c.status].length > 0 && (
                      <form action={transitionChangeAction} className="flex gap-1">
                        ...
                        {NEXT[c.status].map((to) => (
                          <button key={to} type="submit" name="to" value={to} className="rounded-md border border-border px-2 py-1 text-xs text-secondary hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none">
                            {t(`st.${to}`)}
                          </button>
                        ))}
                      </form>
