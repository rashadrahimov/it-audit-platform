...
export default async function PlansPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 p-6 pt-12">
      ...
      {/* Создание плана */}
      <form action={createPlanAction} data-testid="plan-create" className="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-white p-4 shadow-sm">
        ...
        <input name="capacityHours" type="number" min="0" placeholder={t('capacity')} className={`w-28 ${inputCls}`} />
        <button
          type="submit"
      ...
      {plans.length === 0 ? (
        ...
        <ul className="flex flex-col gap-4" data-testid="plans-list">
          {plans.map(({ detail, capacity }) => {
            ...
            return (
              <li key={detail.id} className="flex flex-col gap-4 rounded-xl border border-border bg-white p-5 shadow-sm">
                ...
                {cap !== null && (
                  <div>
                    <div className="mb-1 flex items-baseline justify-between text-sm">
                      ...
                      {over ? (
                        <span className="text-sm font-medium text-destructive">{t('overallocated')}</span>
                      ) : (
                ...
                {detail.items.length > 0 && (
                  <ul className="flex flex-col gap-1" data-testid="plan-items">
                    {detail.items.map((it) => (
                      <li key={it.id} className="flex items-center justify-between gap-2 rounded-md bg-muted/40 px-3 py-1.5 text-sm">
                        ...
                        <span className="flex items-center gap-3 text-xs text-secondary">
                          {it.priorityScore !== null && (
                            <span className="rounded-full bg-white px-2 py-0.5">{t('priority')}: {it.priorityScore}</span>
                          )}
                          <span>{it.plannedHours ?? 0} {t('hours')}</span>
                        </span>
                      </li>
                    ))}
                  </ul>
                ...
                <form action={addPlanItemAction} className="flex flex-wrap items-end gap-2">
                  ...
                  <select name="auditableEntityId" required defaultValue="" className={`flex-1 ${inputCls} py-1.5 text-xs`}>
                    <option value="" disabled>{t('pickEntity')}</option>
                    {universe.map((u) => (
                      <option key={u.id} value={u.id}>{resolveLocalized(u.nameI18n, locale)} ({u.kind})</option>
                    ))}
                  </select>
                  <input name="plannedHours" type="number" min="0" placeholder={t('hours')} className={`w-20 ${inputCls} py-1.5 text-xs`} />
                  <button
                    type="submit"
