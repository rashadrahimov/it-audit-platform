...
export default async function TrustCenterPage() {
  ...
  const [t, tenantSlug] = await Promise.all([getTranslations('trustCenter'), getActiveTenantSlug()]);
        ...
        <>
          <section className="rounded-xl border border-border bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-xs text-secondary">
                  {t('slug')}: <code className="rounded bg-muted px-1.5 py-0.5">/trust/{tc.slug}</code>
                </p>
              </div>
              <span
            ...
            <p className="mt-4 text-sm text-secondary">
              <span className="font-medium text-foreground">{requestCount}</span> {t('accessRequests')}
            </p>
          </section>

          <section>
            <h3 className="mb-3 text-sm font-semibold text-secondary">{t('items')}</h3>
            <ul className="overflow-hidden rounded-xl border border-border bg-white shadow-sm" data-testid="trust-items">
              {tc.items.map((item) => (
                <li
                  key={item.id}
