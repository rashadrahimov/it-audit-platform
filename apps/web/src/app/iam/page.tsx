    ...
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-8 p-6 pt-12">
                ...
                {accounts.map((a) => (
                  <tr key={a.id} className="border-b border-border last:border-0">
                    <td className="px-4 py-3">
                      ...
                      {a.displayName && <span className="ml-2 text-xs text-secondary">{a.displayName}</span>}
                    </td>
                    <td className="px-4 py-3 text-secondary">{t(`tp.${a.type}`)}</td>
                    <td className="px-4 py-3 text-secondary">{a.mfaEnabled === null ? '—' : a.mfaEnabled ? '✓' : '✕'}</td>
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          a.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-secondary'
                        }`}
                      >
                        {t(`st.${a.status}`)}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      {a.status === 'active' && (
                        <form action={deactivateAccountAction}>
                          ...
                          <button type="submit" className={btnCls}>{t('deactivate')}</button>
                        </form>
                      )}
                    </td>
                  </tr>
      ...
      <section className="flex flex-col gap-3">
        ...
        {requests.length === 0 ? (
          ...
          <ul className="overflow-hidden rounded-xl border border-border bg-white shadow-sm" data-testid="access-requests-list">
            {requests.map((r) => (
                  ...
                  {r.status === 'pending' && (
                    <>
                      <form action={decideAccessRequestAction}>
                        ...
                        <button type="submit" className={btnCls}>{t('approve')}</button>
                      </form>
                      <form action={decideAccessRequestAction}>
                        <input type="hidden" name="id" value={r.id} />
                        <input type="hidden" name="decision" value="reject" />
                        <button type="submit" className={btnCls}>{t('reject')}</button>
                      </form>
                    </>
                  )}
                </span>
