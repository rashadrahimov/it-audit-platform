...
- **Текущая задача:** автономная сессия 18.07.2026 — T-011 (RLS) в работе.
- **Следующий шаг:** T-011 → T-013 (локальные аккаунты) / T-018 (RBAC-модель) / T-021 (скелеты+AuditLog).
- **Последнее готово:** **T-010 закрыта** (Drizzle-схема tenant+subsidiary по утверждённому ERD, миграции с откатом, seed доменных данных). За сессию также: T-042 (файлы), T-041 (email), T-040 (BullMQ), T-007 (dev:up), T-006 (compose); T-043 `[!]`. GitHub-remote ещё не заведён. Открыто у заказчика: Excel-шаблоны, регулятор, модель оплаты.
