...
- **Текущая задача:** T-024 закрыта (SAML SSO против Keycloak). Следующая — T-025 (LDAP; понадобится тестовый LDAP-сервер в compose — Keycloak тут не поможет) или UI-сессия (промт D).
- **Следующий шаг:** T-025 → UI-сессии (T-022 i18n-каркас, T-030). Решения по `[!]`: T-012 (нужна membership_scope), T-043 (deps += T-010/доменные поля).
- **Последнее готово:** **T-024 закрыта** (node-saml v5, конфиг IdP из метадаты, POST-callback с подписями/audience/InResponseTo-replay-защитой, JIT через общий SsoUserService, SAML-клиент в realm-json). До того T-016 (OIDC SSO, полный code-flow e2e). GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
