...
- **Текущая задача:** T-031 закрыта (Control: миграция 0010, 16 доменов + 31 контроль из шаблона клиента, маппинги, /controls в UI). Следующая — T-032 (control detail: owner, history, comments — drawer).
- **Следующий шаг:** T-032 → T-035 (Engagement) → M1-цепочка. Решения по `[!]`: T-012 (deps += T-013, T-020?), T-043 (deps += доменные поля finding/test).
- **Последнее готово:** **Марафон 18.07.2026: T-024 (SAML), T-025 (LDAP), T-022 (i18n), T-047 (UI-логин), T-030 (Framework), T-031 (Control)**. GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
