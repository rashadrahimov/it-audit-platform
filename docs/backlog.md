...
- **Текущая задача:** автономная сессия 18.07.2026 — T-017 закрыта; следующая — T-026 (лицензии) или T-021/T-022/T-014.
- **Следующий шаг:** T-026 → T-021 (AuditLog) → T-022 (i18n) → T-014 (TOTP). T-012 `[!]`: deps выполнены, нужна membership_scope — ждёт решения Рашада.
- **Последнее готово:** **T-017 закрыта** (offboarding-эндпоинты + суточная джоба авто-деактивации; тест + e2e). За сессию: T-006→007→040→041→042→010→011→013→018→019→020→017; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
