...
- **Текущая задача:** автономная сессия 18.07.2026 — T-020 закрыта; следующая — T-017 (авто-деактивация).
- **Следующий шаг:** T-017 → T-026 (лицензии) → T-021 (AuditLog) → T-022 (i18n) → T-014 (TOTP). T-012 `[!]`: предложенные deps (T-011/013/020) теперь выполнены, но нужна membership_scope-таблица — ждёт решения Рашада.
- **Последнее готово:** **T-020 закрыта** (membership, PermissionGuard+@RequirePermission, /rbac/check, демо-юзеры; DoD-тест 4 шт. + e2e 403/201/400). За сессию: T-006→007→040→041→042→010→011→013→018→019→020; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
