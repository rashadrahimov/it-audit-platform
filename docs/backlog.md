...
- **Текущая задача:** T-036 закрыта (чеклист: снапшоты контролей, bulk-добавление, UI-форма). Следующая — T-037 (ответы респондентов: response с compliance_status).
- **Следующий шаг:** T-037 → T-038 (Finding; deps T-043`[!]` — вероятно, придётся `[!]` или решать T-043). Решения по `[!]`: T-012, T-043 (deps += доменные поля finding/test).
- **Последнее готово:** **Марафон 18.07.2026: T-024, T-025, T-022, T-047, T-030, T-031, T-032, T-035, T-036 (9 задач)**. GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
