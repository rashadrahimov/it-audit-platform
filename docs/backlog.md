...
- **Текущая задача:** T-025 закрыта (LDAP-федерация, auth-эпик MVP полностью готов). Следующая — T-022 (i18n-каркас, мультиязычные поля) — открывает UI-задачи.
- **Следующий шаг:** T-022 → UI-сессии (T-030+). Решения по `[!]`: T-012 (нужна membership_scope), T-043 (deps += T-010/доменные поля).
- **Последнее готово:** **T-025 закрыта** (openldap в compose, ldapts, search+bind AD-паттерн, экранирование фильтра, JIT через SsoUserService). До того T-024 (SAML), T-016 (OIDC). GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
