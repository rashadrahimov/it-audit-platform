...
- **Текущая задача:** автономная сессия 18.07.2026 — T-023 закрыта; следующая — T-015 (invite) / T-014 (TOTP); T-022 и T-030+ содержат UI → по правилам отдельная сессия (промт D).
- **Следующий шаг:** T-015 → T-014; потом UI-сессии (T-022 i18n-каркас, T-030 список фреймворков). T-012 `[!]`: deps выполнены, нужна membership_scope — ждёт решения Рашада.
- **Последнее готово:** **T-023 закрыта** (soft-delete дочек + restore, полиморфные comments, всё в audit_log). За сессию: T-006→007→040→041→042→010→011→013→018→019→020→017→026→021→023; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
