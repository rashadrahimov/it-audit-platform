...
- **Текущая задача:** T-030 закрыта (Framework: миграция 0009, сид ISO/COBIT/NIST, GET /frameworks, страница /frameworks). Следующая — T-031 (Control: сущность + библиотека + маппинг Control↔Framework).
- **Следующий шаг:** T-031 → T-032 (control detail) → M1-цепочка. Решения по `[!]`: T-012 (deps += T-013, T-020?), T-043 (deps += доменные поля finding/test).
- **Последнее готово:** **Марафон 18.07.2026: T-024 (SAML), T-025 (LDAP), T-022 (i18n), T-047 (UI-логин), T-030 (Framework)**. GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
