...
- **Текущая задача:** автономная сессия 18.07.2026 — T-015 закрыта; следующая — T-014 (TOTP MFA).
- **Следующий шаг:** T-014; потом UI-сессии (T-022 i18n-каркас, T-030 список фреймворков — промт D) и SSO-блок (T-016/024/025 — нужен тестовый IdP в compose). T-012 `[!]`: deps выполнены, нужна membership_scope — ждёт решения Рашада.
- **Последнее готово:** **T-015 закрыта** (invite-flow целиком: письмо с токеном, accept, логин приглашённого; LOG-05 пишется, квоты метрятся). За сессию: T-006→007→040→041→042→010→011→013→018→019→020→017→026→021→023→015; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
