...
- **Текущая задача:** — (автономная сессия 18.07.2026 завершена; T-014 закрыта последней).
- **Следующий шаг:** на выбор Рашада: (а) UI-сессия промтом D — T-022 i18n-каркас или T-030 список фреймворков (M1 стартует); (б) SSO-блок T-016/024/025 — сначала добавить тестовый IdP (Keycloak?) в docker-compose; (в) решения по `[!]`: T-012 (нужна membership_scope), T-043 (deps += T-010/доменные поля).
- **Последнее готово:** **автономная сессия 18.07.2026 — 18 задач**: T-006→007→040→041→042 (инфраструктура: compose, dev:up, BullMQ, email, файлы) → T-010→011 (Drizzle-миграции с откатом, RLS) → T-013→018→019→020→017→026→021→023→015→014 (auth: пароли/политики/lockout, RBAC-матрица+enforcement, деактивация, лицензии, журналы, soft-delete+comments, invite, TOTP MFA); T-003 синхронизирована ([x] по утверждению в data-model.md). M-1 и M0 полностью закрыты, из «Аутентификации» остались только SSO (T-016/24/25). GitHub-remote не заведён (CI потребует postgres-service для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
