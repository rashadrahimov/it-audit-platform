...
- **Одна сессия ≈ одна задача или один вертикальный срез**, а не весь проект. Закрыл T-0XX и проверил — новая сессия.
...
и git log -5. Работай без меня по правилам CLAUDE.md (раздел «Автономный
режим»): бери следующую незаблокированную задачу; для каждой — план → код →
/verify → [x] в бэклоге → git commit. Вопросов не задавай: если задача требует
моего решения — пометь её [!] с причиной и возьми следующую. Максимум 2 задачи,
потом ритуал завершения и промт следующей сессии.
```
