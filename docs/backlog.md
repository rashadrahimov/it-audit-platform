...
- **Текущая задача:** T-047 закрыта (UI-логин: cookie-сессия, MFA-шаг, /account, дизайн-система). Следующая — T-030 (Framework: сущность+сид ISO27001/COBIT/NIST+версионирование, список в UI — вход теперь есть).
- **Следующий шаг:** T-030 → T-031 (Control) → M1-цепочка. Решения по `[!]`: T-012 (deps += T-013, T-020?), T-043 (deps += доменные поля finding/test).
- **Последнее готово:** **Марафон 18.07.2026: T-024 (SAML), T-025 (LDAP — auth-эпик целиком), T-022 (i18n), T-047 (UI-логин, дизайн-система в design-system/)**. GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
