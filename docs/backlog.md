...
- **Текущая задача:** T-033 закрыта (Test-слой: прогоны двигают статус, failing_entities, секция в карточке контроля). Следующая — T-038 (Finding: все колонки чеклиста клиента).
- **Следующий шаг:** T-038 → T-043 (SLA-джоба) → T-039 (lifecycle+письма) → T-012 → T-044 → T-045. Открыто у заказчика: Excel-шаблоны, регулятор, оплата (гейтят остаток T-001).
- **Последнее готово:** **Марафон-2 18.07.2026: T-033.** До того марафон-1 — 12 задач (auth целиком + вертикальный срез M1) и сессия решений (T-043 перевёрнута, T-012 разблокирована, GitHub+CI зелёный).
...
- [ ] **T-038** — Finding: severity(risk), owner с обеих сторон, deadline, resolution date, linked control, remediation, ссылка на стандарт. _Deps: T-037 (T-043 убрана 18.07.2026: поля закладываются статично, авто-пересчёт придёт с T-043)._ DoD: finding создаётся со всеми колонками клиента.
