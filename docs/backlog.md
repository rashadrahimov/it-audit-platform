...
- **Текущая задача:** автономная сессия 18.07.2026 — T-026 закрыта; следующая — T-021 (AuditLog) или T-022/T-014/T-015.
- **Следующий шаг:** T-021 (AuditLog+auth_event) → T-022 (i18n) → T-015 (invite) → T-014 (TOTP). T-012 `[!]`: deps выполнены, нужна membership_scope — ждёт решения Рашада.
- **Последнее готово:** **T-026 закрыта** (license-таблица с RLS, метринг, GET /license/usage, POST /subsidiaries с мягким предупреждением о квоте). За сессию: T-006→007→040→041→042→010→011→013→018→019→020→017→026; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
