...
- **Текущая задача:** автономная сессия 18.07.2026 — T-013 закрыта, дальше T-018 (RBAC-модель) / T-021 / T-022 / T-026.
- **Следующий шаг:** T-018→T-019→T-020 (RBAC-цепочка) — разблокирует T-012; параллельно открыты T-014 (TOTP), T-016/24/25 (SSO), T-017, T-021, T-022, T-026.
- **Последнее готово:** **T-013 закрыта** (user-таблица, scrypt, JWT+guard, политика per tenant, lockout, смена пароля — всё e2e-проверено). За сессию: T-006→007→040→041→042→010→011→013; `[!]`: T-012 (нужны membership+enforcement), T-043 (нужны доменные поля). GitHub-remote не заведён (CI понадобится postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
