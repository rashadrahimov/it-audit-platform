...
- **Текущая задача:** T-022 закрыта (i18n: shared-контракт + next-intl-каркас). M0 остался: T-012 `[!]`, T-043 `[!]` (оба ждут решения Рашада). Следующая — T-030+ (UI-экраны, промт D со скиллом ui-ux-pro-max) или разбор `[!]`.
- **Следующий шаг:** UI-сессии (T-030+); решения по `[!]`: T-012 (deps += T-013, T-020?), T-043 (deps += доменные поля finding/test).
- **Последнее готово:** **Марафон 18.07.2026: T-024 (SAML), T-025 (LDAP — auth-эпик готов целиком), T-022 (i18n)**. GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
