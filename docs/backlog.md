...
- **Текущая задача:** автономная сессия 18.07.2026 — T-011 закрыта, дальше T-012 (групповой уровень) или T-013/T-018/T-021/T-022.
- **Следующий шаг:** T-012 (агрегаты по группе) → T-013 (локальные аккаунты) → T-021 (скелеты+AuditLog).
- **Последнее готово:** **T-011 закрыта** (RLS: роль `app` без суперправ, FORCE-политики, `DbService.withTenant`, 4 интеграционных теста изоляции). За сессию: T-006→T-007→T-040→T-041→T-042→T-010→T-011; T-043 `[!]`. GitHub-remote не заведён (CI понадобится postgres-service для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, модель оплаты.
