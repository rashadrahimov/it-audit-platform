...
- **Текущая задача:** T-038 закрыта (Finding: все колонки клиента, owner двух сторон, секция в engagement). Следующая — T-043 (SLA-джоба: deps T-040 ✓, T-038 ✓ — разблокирована).
- **Следующий шаг:** T-043 → T-039 (lifecycle+письма) → T-012 → T-044 → T-045. Открыто у заказчика: Excel-шаблоны, регулятор, оплата (гейтят остаток T-001).
- **Последнее готово:** **Марафон-2 18.07.2026: T-033, T-038.** До того марафон-1 — 12 задач и сессия решений (GitHub+CI зелёный).
...
- [ ] **T-043** — SLA-примитивы (B15): фоновая джоба-пересчёт авто-статусов (OK/Due soon/Overdue) по полям `due_date`/`sla_status` сущностей finding/test. _Deps: T-040, T-038 (решение Рашада 18.07.2026: зависимость ПЕРЕВЁРНУТА — сущности создаются со своими полями статично в T-033/T-038, джоба приходит после и пересчитывает; прежнее предложение создавало цикл)._ DoD: статус пересчитывается по времени.
