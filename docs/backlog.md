...
- **Текущая задача:** автономная сессия 18.07.2026 — T-018 закрыта; следующая — T-019 (пресет-роли) или T-021/T-022/T-026.
- **Следующий шаг:** T-019 (пресеты, потребует membership для аудит-иерархии — смотреть по месту) → T-020 (enforcement) — разблокирует T-012; параллельно открыты T-014 (TOTP), T-017, T-021, T-022, T-026.
- **Последнее готово:** **T-018 закрыта** (permission/role/role_permission с RLS, каталог 30 прав + демо-роль с матрицей в seed, чтение /rbac/* под JWT). За сессию: T-006→007→040→041→042→010→011→013→018; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
