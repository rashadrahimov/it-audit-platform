...
- **Текущая задача:** T-016 закрыта (OIDC SSO через Keycloak). Следующая — T-024 (SAML) или T-025 (LDAP): Keycloak уже в compose и умеет SAML; либо UI-сессия (промт D).
- **Следующий шаг:** T-024/T-025 (тестовый IdP уже есть) → UI-сессии (T-022 i18n-каркас, T-030). Решения по `[!]`: T-012 (нужна membership_scope), T-043 (deps += T-010/доменные поля).
- **Последнее готово:** **T-016 закрыта** (Keycloak в compose с автоимпортом realm, openid-client v6/discovery, полный code-flow e2e, JIT-провижининг). До того автономная сессия 18.07.2026 — 18 задач (M-1+M0 целиком, auth-эпик почти весь). GitHub-remote не заведён (CI потребует postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
