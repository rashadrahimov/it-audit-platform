...
- **Текущая задача:** автономная сессия 18.07.2026 — T-021 закрыта; следующая — T-023 (soft-delete+comments) / T-022 (i18n) / T-015 (invite) / T-014 (TOTP).
- **Следующий шаг:** T-022 → T-015 → T-014. T-012 `[!]`: deps выполнены, нужна membership_scope — ждёт решения Рашада.
- **Последнее готово:** **T-021 закрыта** (audit_log + auth_event: append-only, RLS-чтение, запись из логина/дочек/деактивации; всё e2e-проверено по БД). За сессию: T-006→007→040→041→042→010→011→013→018→019→020→017→026→021; `[!]`: T-012, T-043. GitHub-remote не заведён (CI: postgres для rls.spec). Открыто у заказчика: Excel-шаблоны, регулятор, оплата.
