...
- **Текущая задача:** Решения Рашада 18.07.2026 зафиксированы: T-043 перевёрнута (deps T-040+T-038; T-033/T-038 освобождены от неё), T-012 разблокирована (deps T-011/T-013/T-020 — выполнены), GitHub-remote заведён + CI с services (postgres/redis/minio). Незаблокированы: **T-033, T-038, T-012**.
- **Следующий шаг:** марафон по finding-хвосту M1: T-033 (Test) → T-038 (Finding) → T-043 (SLA-джоба) → T-039 (lifecycle+письма) → T-012 (групповые агрегаты) → T-044 (дашборды) → T-045 (экспорт). Открыто у заказчика: Excel-шаблоны, регулятор, оплата (гейтят остаток T-001).
- **Последнее готово:** **Марафон 18.07.2026 — 12 задач** (T-024/025/022/047/030/031/032/035/036/037/046/034): auth-эпик целиком + вертикальный срез M1 (библиотека → engagement → чеклист → ответы → evidence). Детали — в строках задач, verify-рецепты — в SKILL.md.
...
- [ ] **T-033** — Test: сущность (Control 1→N Test), статус, owner, due date, SLA, «failing entities». _Deps: T-031 (T-043 убрана 18.07.2026: поля due_date/sla_status закладываются статично, авто-пересчёт придёт с T-043)._ DoD: тест привязан к контролю, статус меняется.
