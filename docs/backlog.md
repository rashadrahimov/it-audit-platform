...
- **Текущая задача:** **Марафон 18.07.2026 завершён: незаблокированных задач не осталось.** Закрыто 12: T-024 (SAML), T-025 (LDAP), T-022 (i18n), T-047 (UI-логин+дизайн-система), T-030 (Framework), T-031 (Control), T-032 (control detail), T-035 (Engagement), T-036 (чеклист), T-037 (ответы), T-046 (onboarding), T-034 (документы). Auth-эпик MVP целиком; вертикальный срез M1: библиотека → engagement → чеклист → ответы → evidence.
- **Следующий шаг:** сессия решений с Рашадом — развязать `[!]`: T-012 (deps += T-013, T-020?), T-043 (deps += доменные поля finding/test → открывает T-033/T-038/T-039/T-044/T-045 — весь finding-хвост M1). Плюс: завести GitHub-remote (CI потребует services: postgres), ответы клиента (Excel-шаблоны, регулятор, оплата) — гейтят остаток T-001.
- **Последнее готово:** см. выше; детали по каждой задаче — в её строке бэклога, verify-рецепты — в .claude/skills/verify/SKILL.md.
